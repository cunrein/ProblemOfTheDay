...
fun parseData(data: List<String>): Map<String, MutableMap<String, BooleanArray>> {
    val gaurds = mutableMapOf<String, MutableMap<String, BooleanArray>>()
    var current = mutableMapOf<String, BooleanArray>()
    var isAsleep = false
    var start = 0
    data.forEach {
        val fields = it.split(" ")
        val yearMonthDay = fields[0].replace("[", "").split("-")
        val time = fields[1].replace("]", "").split(":")
        var dateTime = LocalDateTime.of(
            2018, yearMonthDay[1].toInt(),
            yearMonthDay[2].toInt(), time[0].toInt(), time[1].toInt()
        )
        ...
        val day =
            "${dateTime.monthValue.toString().padStart(2, '0')}-${dateTime.dayOfMonth.toString().padStart(2, '0')}"
        val minute = time[1].toInt()

        if (fields[3].contains("#")) {
            val key = fields[3].replace("#", "")
            current = if (gaurds.containsKey(key)) {
                if (!gaurds[key]!!.containsKey(day)) {
                    addDayPair(day, gaurds, key)
                }
                gaurds[key]!!
            } else {
...
private fun createDayPair(
    day: String,
    gaurds: MutableMap<String, MutableMap<String, BooleanArray>>,
    key: String
) {
    ...
    val map = mutableMapOf(Pair(day, booleanList.toBooleanArray()))
    gaurds[key] = map
}

private fun addDayPair(
    day: String,
    gaurds: MutableMap<String, MutableMap<String, BooleanArray>>,
    key: String
) {
    ...
    gaurds[key]!![day] = booleanList.toBooleanArray()
}

val dayFourTestData = listOf(
